...
std::unique_ptr<Command> Smt2CmdParser::parseNextCommand()
{
  ...
  switch (tok)
  {
    ...
    break;
    // anynomous author: optimize: maximize / minimize
    case Token::MAXIMIZE_TOK:{
        // next command
        Term t = d_tparser.parseTerm();
        cmd.reset(new OptimizeCommand(t, true));
      }
      break;
    case Token::MINIMIZE_TOK:{
        Term t = d_tparser.parseTerm();
        cmd.reset(new OptimizeCommand(t, false));
      }
      break;
    case Token::GET_OBJECTIVES_TOK:{
        cmd.reset(new GetObjectivesCommand());
      }
      break;
    case Token::EOF_TOK:
