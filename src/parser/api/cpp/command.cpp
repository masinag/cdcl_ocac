...
/* -------------------------------------------------------------------------- */
/* anynomous author: class OptimizeCommand                                                        */
/* -------------------------------------------------------------------------- */

OptimizeCommand::OptimizeCommand(const cvc5::Term& t, bool isMin) : d_term(t), _isMinimize(isMin) {}

cvc5::Term OptimizeCommand::getTerm() const { return d_term; }
bool OptimizeCommand::isMinimize() const { return _isMinimize; }
void OptimizeCommand::invoke(cvc5::Solver* solver, SymbolManager* sm)
{
  try
  {
    if(_isMinimize){
      solver->minimizeFormula(d_term);
    }
    else{
      solver->maximizeFormula(d_term);
    }
    // solver->assertFormula(d_term);
    d_commandStatus = CommandSuccess::instance();
  }
...
std::string OptimizeCommand::getCommandName() const { return _isMinimize?"minimize":"maximize"; }

void OptimizeCommand::toStream(std::ostream& out) const
{
  _isMinimize?Printer::getPrinter(out)->toStreamCmdMinimize(out, termToNode(d_term)):Printer::getPrinter(out)->toStreamCmdMaximize(out, termToNode(d_term));
}


/* -------------------------------------------------------------------------- */
/* class GetObjectivesCommand                                                      */
/* -------------------------------------------------------------------------- */

GetObjectivesCommand::GetObjectivesCommand() {}
void GetObjectivesCommand::invoke(cvc5::Solver* solver, SymbolManager* sm)
{
  try
  {
    d_result = solver->getObjectives();
...
void GetObjectivesCommand::printResult(cvc5::Solver* solver, std::ostream& out) const
{
  out << d_result;
}

std::string GetObjectivesCommand::getCommandName() const { return "get-objective"; }

void GetObjectivesCommand::toStream(std::ostream& out) const
{
  Printer::getPrinter(out)->toStreamCmdGetObjectives(out);
}
