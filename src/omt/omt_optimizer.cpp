...
bool OMTOptimizer::nodeSupportsOptimization(TNode node)
{
  ...
  return (type.isInteger() || type.isBitVector() || type.isReal()); // anynomous author: add real node support
}
    ...
    const OptimizationObjective& objective)
{
  ...
  if (objectiveType.isInteger() || objectiveType.isReal()){
    // int/real type: use OMTOptimizerMCSAT
    return std::unique_ptr<OMTOptimizer>(new OMTOptimizerMCSAT());
  }
  ...
  // else if (objectiveType.isReal()){
  //   // real type: use OMTOptimizerMCSAT
  //   return std::unique_ptr<OMTOptimizer>(new OMTOptimizerMCSAT());
  // }
  else
  {
    ...
    const OptimizationObjective& objective)
{
  ...
  switch (objective.getType())
  {
    case OptimizationObjective::MINIMIZE:
    {
      ...
      else if (targetType.isReal())
      {
        Assert(lhs.getType().isReal()) << lhsTypeError;
        Assert(rhs.getType().isReal()) << rhsTypeError;
        return nm->mkNode(Kind::LT, lhs, rhs);
      }
      else
      {
