  ...
  case kind::GEQ:
    {
      ...
      // anynomous author: for REAL_ALGEBRAIC_NUMBER
      if(atom[1].getKind() == Kind::REAL_ALGEBRAIC_NUMBER){
        // note that atom[1].getOperator(): from REAL_ALGEBRAIC_NUMBER to REAL_ALGEBRAIC_NUMBER_OP
        const RealAlgebraicNumber& p = atom[1].getOperator().getConst<RealAlgebraicNumber>();
        const Rational& q = p.toRational();
