...
void TheoryArith::postCheck(Effort level)
{
  ...
  // if(!isOptimization()){ // anynomous author: unsat lemma will be escaped.
  ...
  if (d_internal->postCheck(level))
  {
    ...
    // anynomous author: because it preregister, x > sqrt(2) -> x > 1.4,. which may lead to a bound error.
  }
  if (d_im.hasSent())
  {
    return;
  ...
  if (Theory::fullEffort(level))
  {
    ...
    if (d_nonlinearExtension != nullptr)
    {
      ...
      // anynomous author: finally check full effort.
      if(Theory::isOptimization()){
        if(!d_nonlinearExtension->isOptimization()){
          d_nonlinearExtension->setOptimization();
        }
        if(Theory::_isOptimization != OPT_TYPE::WAITING){
          d_nonlinearExtension->setObjective(Theory::_isOptimization, objectiveVariable);
        }
      }
      d_nonlinearExtension->checkFullEffort(d_arithModelCache, termSet);
      // std::cout<<"d_nonlinearExtension"<<std::endl;
                                     ...
                                     const std::set<Node>& termSet)
{
  ...
  if(isOptimization()){
    ...
    std::map<Node, Node> optCache;
    d_nonlinearExtension->getOptModelCache(optCache);
    for (const std::pair<const Node, Node>& p : optCache)
    {
...
void TheoryArith::assertOptBound(){
  d_nonlinearExtension->assertOptBound();
}
bool TheoryArith::missBound() const{
  return d_nonlinearExtension->missBound();
}
Node TheoryArith::getBoundValue(const Node& e) const{
  return d_nonlinearExtension->getBoundValue(e);
}
Node TheoryArith::getOptimizedValue(const Node& e) const{
  return d_nonlinearExtension->getOptimizedValue(e);
}
bool TheoryArith::isOptSAT() const{
  return d_nonlinearExtension->isOptSAT();
}
void TheoryArith::buildOptModel(TheoryModel* m){
  std::set<Node> termSet;
  collectAssertedTermsForModel(termSet);
  // also get relevant terms
  computeRelevantTerms(termSet);
  collectModelInfo(m, termSet);
}
bool TheoryArith::isOptUnbounded() const{
  return d_nonlinearExtension->isOptUnbounded();
}
bool TheoryArith::isOptPosInf() const{
  return d_nonlinearExtension->isOptPosInf();
}
bool TheoryArith::isOptNegInf() const{
  return d_nonlinearExtension->isOptNegInf();
}
bool TheoryArith::isOptPosApprox() const{
  return d_nonlinearExtension->isOptPosApprox();
}
bool TheoryArith::isOptNegApprox() const{
  return d_nonlinearExtension->isOptNegApprox();
}
