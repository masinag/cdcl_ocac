                                         ...
                                         const std::set<Node>& termSet)
{
  ...
  if (!hasNlTerms())
  {
    ...
    // anynomous author: if it is optimization, go on checking ...
    ...
    const std::set<Node>& termSet)
{
  ...
  do
  {
    ...
    // a wrong value
    // anynomous author: TODO, isOptimization or have false assertions
    if (!false_asserts.empty() || isOptimization())
    // if (!false_asserts.empty())
    {
      complete_status = 0;
      runStrategy(Theory::Effort::EFFORT_FULL, assertions, false_asserts, xts); // anynomous author: TODO entry of strategy solver
                                     ...
                                     const std::vector<Node>& xts)
{
  ...
  {
    if(isOptimization()){
      // std::cout<<"runStrategy: isOptimization"<<std::endl;
      d_strategy.initializeOptimizationStrategy(); // anynomous author: TODO, how to intialize strategy
    }
    else d_strategy.initializeStrategy(options());
  }
  ...
  while (!stop && steps.hasNext())
  {
    ...
    switch (step)
    {
      ...
      case InferStep::COVERINGS_INIT: 
        {
          if(isOptimization()){
            d_covSlv.setOptimization();
            if(_isOptimization != OPT_TYPE::WAITING) 
              d_covSlv.setObjective(_isOptimization, objectiveVariable);
          }
          d_covSlv.initLastCall(assertions); 
        }
      break;
...
void NonlinearExtension::setOptimization(){
  _isOptimization = OPT_TYPE::WAITING;
  // std::cout<<"NonlinearExtension::setOptimization"<<std::endl;
}
void NonlinearExtension::setObjective(OPT_TYPE _isOpt, Node& e){
  // std::cout<<"NonlinearExtension::setObjective"<<std::endl;
  _isOptimization = _isOpt;
  objectiveVariable = e;
}
bool NonlinearExtension::isOptimization() const{
  return _isOptimization != OPT_TYPE::NON_OPT;
}

void NonlinearExtension::assertOptBound(){
  return d_covSlv.assertOptBound();
}
bool NonlinearExtension::missBound() const{
  return d_covSlv.missBound();
}
Node NonlinearExtension::getBoundValue(const Node& e) const{
  return d_covSlv.getBoundValue(e);
}
Node NonlinearExtension::getOptimizedValue(const Node& e) const{
  return d_covSlv.getOptimizedValue(e);
}
bool NonlinearExtension::isOptSAT() const{
  return d_covSlv.isOptSAT();
}

void NonlinearExtension::getOptModelCache(std::map<Node, Node>& optCache){
  d_covSlv.getOptModelCache(optCache);
}
bool NonlinearExtension::isOptUnbounded() const{
  return d_covSlv.isOptUnbounded();
}
bool NonlinearExtension::isOptPosInf() const{
  return d_covSlv.isOptPosInf();
}
bool NonlinearExtension::isOptNegInf() const{
  return d_covSlv.isOptNegInf();
}
bool NonlinearExtension::isOptPosApprox() const{
  return d_covSlv.isOptPosApprox();
}
bool NonlinearExtension::isOptNegApprox() const{
  return d_covSlv.isOptNegApprox();
}
