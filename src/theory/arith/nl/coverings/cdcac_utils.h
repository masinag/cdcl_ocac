...
bool sampleOutsidewithDirection(const std::vector<CACInterval>& infeasible, poly::Value& sample, bool isMinimize);
...
//  anynomous author: Traverse the infeasible intervals and check whether there is only one interval;
//  When getSatIntervals, if there is (-oo, +oo), it means opt can not resolve such ''conflict'';
//  So that such interval stored in unbounded_proof is the final satisfiable cell;
//  We should analyze such interval via optimization direction: (a, b) -> min: a+\epsilon, max: b-\epsilon;
//  Note that it is an open interval as in sampleOutsidewithDirection, we have already attempted bounds. 
bool checkUnbounded(const std::vector<CACInterval>& infeasibles, poly::Interval& proof);

poly::SignCondition ConditionNot(const poly::SignCondition& sc);

// anynomous author: a - b
void cleanIntervals(std::vector<poly::Interval>& intervals);
std::vector<poly::Interval> intervalMinus(const poly::Interval& a, const poly::Interval& b);
std::vector<poly::Interval> intervalMinus(const std::vector<poly::Interval>& a, const std::vector<poly::Interval>& b);
std::vector<poly::Interval> intervalMinus(const std::vector<poly::Interval>& a, const poly::Interval& b);
std::vector<poly::Interval> intervalMinus(const poly::Interval& a, const std::vector<poly::Interval>& b);
