        ...
        if( theoryId!=THEORY_QUANTIFIERS ){
          ...
          if (theory && isTheoryEnabled(theoryId))
          {
            if( theory->needsCheckLastEffort() ){
              ...
              // anynomous author: report whther it is optimization
              if(isOptimization()){
                // anynomous author: directly setting theory[THEORY_ARITH]->setOptimization()
                if(!theory->isOptimization()) theory->setOptimization(); // anynomous author: just set optimization
                
                if(_isOptimization != OPT_TYPE::WAITING) theory->setObjective(_isOptimization, objectiveVariable);
              }
              theory->check(Theory::EFFORT_LAST_CALL); // anynomous author: TODO real entry of theory::check()
            }
          }
        }
      }
...
void TheoryEngine::setOptimization(){
  _isOptimization = OPT_TYPE::WAITING;
  // std::cout<<"TheoryEngine::setOptimization"<<std::endl;
  // anynomous author: must set THEORY_ARITH optimization here, or it will first not set optimization
  //      and set optimization at second time.
  //      In the future, we can set the traverse the theories and set optimization flag.
  Theory* theory = d_theoryTable[THEORY_ARITH];
  theory->setOptimization();
  // std::cout<<"theory engine setOptimization()"<<std::endl;
  // std::cout<<"theory THEORY_ARITH setOptimization() "<<theory->isOptimization()<<std::endl;
}
void TheoryEngine::setObjective(OPT_TYPE _isOpt, Node& e){
  // std::cout<<"TheoryEngine::setObjective"<<std::endl;
  ...
  //      In the future, we can set the traverse the theories and set optimization flag.
  Theory* theory = d_theoryTable[THEORY_ARITH];
  theory->setObjective(_isOpt, e);
  // std::cout<<"theory engine setOptimization()"<<std::endl;
  // std::cout<<"theory THEORY_ARITH setOptimization() "<<theory->isOptimization()<<std::endl;
}
bool TheoryEngine::isOptimization() const{
  return _isOptimization != OPT_TYPE::NON_OPT;
}
/**
 * anynomous author: optimization functions, TODO, whether they are right.
 *
 * @throw ModalException, TypeCheckingException, LogicException
 */
void TheoryEngine::assertOptBound(){
  Theory* theory = d_theoryTable[THEORY_ARITH];
  return theory->assertOptBound();
}
bool TheoryEngine::missBound() const{
  Theory* theory = d_theoryTable[THEORY_ARITH];
  return theory->missBound();
}
Node TheoryEngine::getBoundValue(const Node& e) const{
  Theory* theory = d_theoryTable[THEORY_ARITH];
  return theory->getBoundValue(e);
}
Node TheoryEngine::getOptimizedValue(const Node& e) const{
  Theory* theory = d_theoryTable[THEORY_ARITH];
  return theory->getOptimizedValue(e);
}
bool TheoryEngine::isOptSAT() const{
  Theory* theory = d_theoryTable[THEORY_ARITH];
  return theory->isOptSAT();
}
void TheoryEngine::buildOptModel(){
  Theory* theory = d_theoryTable[THEORY_ARITH];
  theory->buildOptModel(d_tc->getModel());
}
bool TheoryEngine::isOptUnbounded() const{
  Theory* theory = d_theoryTable[THEORY_ARITH];
  return theory->isOptUnbounded();
}
bool TheoryEngine::isOptPosInf() const{
  Theory* theory = d_theoryTable[THEORY_ARITH];
  return theory->isOptPosInf();
}
bool TheoryEngine::isOptNegInf() const{
  Theory* theory = d_theoryTable[THEORY_ARITH];
  return theory->isOptNegInf();
}
bool TheoryEngine::isOptPosApprox() const{
  Theory* theory = d_theoryTable[THEORY_ARITH];
  return theory->isOptPosApprox();
}
bool TheoryEngine::isOptNegApprox() const{
  Theory* theory = d_theoryTable[THEORY_ARITH];
  return theory->isOptNegApprox();
}
