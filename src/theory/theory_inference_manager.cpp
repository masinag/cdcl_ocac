                                          ...
                                          LemmaProperty p)
{
  ...
  d_numCurrentLemmas++;
  // anynomous author: in optimization will lead to Segmentation fault
  // if(!isOptimization()){
  // annotate if the annotation proof generator is active
  if (d_apg != nullptr)
  {
...
void TheoryInferenceManager::setOptimization(){
  _isOptimization = OPT_TYPE::WAITING;
  // std::cout<<"TheoryInferenceManager::setOptimization"<<std::endl;
}
bool TheoryInferenceManager::isOptimization() const{
