...
#include "smt/optimization_solver.h" // anynomous author: optimization solver
...
Solver::Solver(std::unique_ptr<internal::Options>&& original)
{
  ...
  // anynomous author: changed
  d_slv = new internal::SolverEngine(d_originalOptions.get());  
  d_slv->setSolver(this);
  // d_slv.reset(new internal::SolverEngine(d_originalOptions.get()));
  // d_slv->setSolver(this);
  d_opt_slv = new internal::smt::OptimizationSolver(d_slv);
...
// anynomous author: optimize formula
void Solver::maximizeFormula(const Term& term)
{
  CVC5_API_TRY_CATCH_BEGIN;
  CVC5_API_SOLVER_CHECK_TERM(term);
  // CVC5_API_SOLVER_CHECK_TERM_WITH_SORT(term, getBooleanSort()); // not check whether it is boolean
  // ensureWellFormedTerm(term); // not check whether it is well formed
  //////// all checks before this line
  // d_slv->assertFormula(*term.d_node);
  // anynomous author: cannot set-optimization after set-logic immediately, because theory solver is not defined
  // d_slv->setOptimization();
  d_opt_slv->addObjective(*term.d_node, internal::smt::OptimizationObjective::MAXIMIZE);
  ////////
  CVC5_API_TRY_CATCH_END;
}
void Solver::minimizeFormula(const Term& term)
{
  CVC5_API_TRY_CATCH_BEGIN;
  CVC5_API_SOLVER_CHECK_TERM(term);
  // CVC5_API_SOLVER_CHECK_TERM_WITH_SORT(term, getBooleanSort()); // not check whether it is boolean
  ...
  // anynomous author: cannot set-optimization after set-logic immediately, because theory solver is not defined
  // d_slv->setOptimization();
  d_opt_slv->addObjective(*term.d_node, internal::smt::OptimizationObjective::MINIMIZE);
  ////////
  CVC5_API_TRY_CATCH_END;
}
std::string Solver::getObjectives()
{
  CVC5_API_TRY_CATCH_BEGIN;
  // anynomous author: 2023-09-04: no need to check sat
...
void Solver::setLogic(const std::string& logic)
{
  CVC5_API_TRY_CATCH_BEGIN;
