...
class CVC5_EXPORT Solver
{
 ...
 public:
  ...
  void assertFormula(const Term& term) const;
    ...
    /**
   * anynomous author: optimize a formula.
   *
   * SMT-LIB?:
   *
   * \verbatim embed:rst:leading-asterisk
   ...
   * \endverbatim
   *
   * @param term The formula to optimize.
   */
   ...
   */
  void setLogic(const std::string& logic); // anynomous author: remove const
 ...
 private:
  ...
  internal::SolverEngine* d_slv; // remove unique pointer
  /** The random number generator of this solver. */
  std::unique_ptr<internal::Random> d_rng;
  /** anynomous author: The OMT solver. */
  bool isOptimization = false;
  internal::smt::OptimizationSolver* d_opt_slv;
};
