...
TheoryModel* SolverEngine::getAvailableModel(const char* c) const
{
  ...
  // anynomous author: if it is not optimization, check whether is able to output model
  if(!isOptimization()){
...
Result SolverEngine::checkSatInternal(const std::vector<Node>& assumptions)
{
  ...
  // anynomous author: here to check sat!!! from engine to driver
...
void SolverEngine::setOptimization(){
  finishInit();
  // std::cout<<"SolverEngine::setOptimization"<<std::endl;
  _isOptimization = OPT_TYPE::WAITING;
  d_smtSolver->setOptimization();
}
bool SolverEngine::isOptimization() const{
  return _isOptimization != OPT_TYPE::NON_OPT;
}

/**
 * anynomous author: optimization functions
 *
 * @throw ModalException, TypeCheckingException, LogicException
 */

void SolverEngine::assertOptBound(){
  d_smtSolver->assertOptBound();
}
void SolverEngine::setObjective(const bool isMinimize, Node& e){
  // std::cout<<"SolverEngine::setObjective"<<std::endl;
  _isOptimization = isMinimize?OPT_TYPE::MINIMIZATION:OPT_TYPE::MAXIMIZATION;
  d_smtSolver->setObjective(isMinimize, e);
}
void SolverEngine::unsetObjective(){
  d_smtSolver->unsetObjective();
}
bool SolverEngine::missBound() const{
  return d_smtSolver->missBound(); // TODO? whether missbound is necessary
  // it is getboundvalue - getoptimizedvalue
}
Node SolverEngine::getBoundValue(const Node& e) const{
  return d_smtSolver->getBoundValue(e);
}
Node SolverEngine::getOptimizedValue(const Node& e) const{
  return d_smtSolver->getOptimizedValue(e);
}
bool SolverEngine::isOptSAT() const{
  return d_smtSolver->isOptSAT();
}
void SolverEngine::buildOptModel(){
  return d_smtSolver->buildOptModel();
}
bool SolverEngine::isOptUnbounded() const{
  return d_smtSolver->isOptUnbounded();
}
bool SolverEngine::isOptPosInf() const{
  return d_smtSolver->isOptPosInf();
}
bool SolverEngine::isOptNegInf() const{
  return d_smtSolver->isOptNegInf();
}
bool SolverEngine::isOptPosApprox() const{
  return d_smtSolver->isOptPosApprox();
}
bool SolverEngine::isOptNegApprox() const{
  return d_smtSolver->isOptNegApprox();
}


// Result SolverEngine::checkSatFromPhase(SolverEngine* preSolver){
