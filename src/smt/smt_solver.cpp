...
void SmtSolver::setOptimization(){
  _isOptimization = OPT_TYPE::WAITING;
  // std::cout<<"SmtSolver::setOptimization"<<std::endl;
  if(d_theoryEngine) d_theoryEngine->setOptimization();
}
bool SmtSolver::isOptimization() const{
 ...
 */
void SmtSolver::setObjective(const bool isMinimize, Node& e){
  // std::cout<<"SmtSolver::setObjective"<<std::endl;
  if(!d_theoryEngine->isOptimization()) setOptimization();
  _isOptimization = isMinimize?OPT_TYPE::MINIMIZATION:OPT_TYPE::MAXIMIZATION;
  d_theoryEngine->setObjective(_isOptimization, e);
  d_pp.setObjective();
}
void SmtSolver::unsetObjective(){
  d_pp.unsetObjective();
}
void SmtSolver::assertOptBound(){
  d_theoryEngine->assertOptBound();
}
bool SmtSolver::missBound() const{
  return d_theoryEngine->missBound(); // TODO? whether missbound is necessary
  // it is getboundvalue - getoptimizedvalue
}
Node SmtSolver::getBoundValue(const Node& e) const{
  return d_theoryEngine->getBoundValue(e);
}
Node SmtSolver::getOptimizedValue(const Node& e) const{
  return d_theoryEngine->getOptimizedValue(e);
}
bool SmtSolver::isOptSAT() const{
  return d_theoryEngine->isOptSAT();
}
void SmtSolver::buildOptModel(){
  return d_theoryEngine->buildOptModel();
}
bool SmtSolver::isOptUnbounded() const{
  return d_theoryEngine->isOptUnbounded();
}
bool SmtSolver::isOptPosInf() const{
  return d_theoryEngine->isOptPosInf();
}
bool SmtSolver::isOptNegInf() const{
  return d_theoryEngine->isOptNegInf();
}
bool SmtSolver::isOptPosApprox() const{
  return d_theoryEngine->isOptPosApprox();
}
bool SmtSolver::isOptNegApprox() const{
  return d_theoryEngine->isOptNegApprox();
}
