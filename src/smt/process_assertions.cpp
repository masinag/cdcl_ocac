...
void ProcessAssertions::setObjective(){
  // std::cout<<"ProcessAssertions::setObjective"<<std::endl;
  _set_objective = true;
}
void ProcessAssertions::unsetObjective(){
...
bool ProcessAssertions::apply(AssertionPipeline& ap)
{
  ...
  // anynomous author: after apply-substs, the objective equation will be transformated.
  if(!_set_objective){
  ...
  // anynomous author: if set the objective equation, we escape the simplification.
  if(!_set_objective){
    ...
    // anynomous author: after simplification, they will discard the objective assertions.
